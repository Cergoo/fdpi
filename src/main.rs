...
struct Cli {
    ...
    nolog: bool,
    /// fuckdpi split pos [example: -s1 -s4] range 1..64
    #[arg(short, long, value_parser = clap::value_parser!(u8).range(1..64))]
    split: Vec<u8>,
    /// fuckdpi disorder pos [example: -d1 -d4] range 1..64
    #[arg(short, long, value_parser = clap::value_parser!(u8).range(1..64))]
    disorder: Vec<u8>,
...
async fn tcp_server(tx: mpsc::Sender<Responder>, addr: SocketAddr, fdpi_methods: (Vec<FdpiItem>, u8)) -> Result<()> {
    // counter
...
fn main() {
    ...
    let fdm = (prepar_fdpi_command(&cli.split, &cli.disorder), cli.ttl); 
...
async fn process(mut socket: &mut TcpStream, tx: mpsc::Sender<Responder>, fdpi_methods: (Vec<FdpiItem>, u8)) -> Result<()> {
